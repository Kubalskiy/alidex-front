...
export var Actions = {
    ...
    fire(action) {
        marketStore.dispatch(action)
    },

    userStatusUpdated(userStatus) {
        return this.createAction("USER_STATUS_UPDATED", userStatus);
    },
    fireUserStatusUpdated(userStatus) {
        this.fire(this.userStatusUpdated(userStatus))
    },

    drawerStateUpdated(drawerState) {
        return this.createAction("DRAWER_STATE_UPDATED", drawerState);
    },
    fireDrawerStateUpdated(drawerState) {
        this.fire(this.drawerStateUpdated(drawerState))
    },

    pageSelected(page) {
        return this.createAction("PAGE_UPDATED", page);
    },
    firePageSelected(page) {
        this.fire(this.pageSelected(page))
    },

    contractLoaded(contract) {
        return this.createAction("CONTRACT_LOADED", contract);
    },
    fireContractLoaded(storefronts) {
        this.fire(this.contractLoaded(storefronts))
    },

    ipfsLoaded(ipfs) {
        return this.createAction("IPFS_LOADED", ipfs);
    },
    fireIpfsLoaded(ipfs) {
        this.fire(this.ipfsLoaded(ipfs))
    },

    accountsLoaded(accounts) {
        return this.createAction("ACCOUNTS_LOADED", accounts);
    },
    fireAccountsLoaded(accounts) {
        this.fire(this.accountsLoaded(accounts))
    },

    storefrontSelected(storefront) {
        return this.createAction("STOREFRONT_SELECTED", storefront);
    },
    fireStorefrontSelected(storefront) {
        this.fire(this.storefrontSelected(storefront))
    }
}
