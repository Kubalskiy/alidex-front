...
class App extends Component {
    ...
    componentDidMount = async () => {
        try {
            ...
            const deployedNetwork = Alidex.networks[networkId];
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        storefrontsFetched(storefronts) {
            dispatch(Actions.storefrontsFetched(storefronts))
        },
        contractLoaded(contract) {
            dispatch(Actions.contractLoaded(contract))
        },
        accountsLoaded(accounts) {
            dispatch(Actions.accountsLoaded(accounts))
        },
        ipfsLoaded(ipfs) {
            dispatch(Actions.ipfsLoaded(ipfs))
        }
    }
}
