...
import {UsersUtil} from '../helpers/users';
import {ContractAccess} from '../helpers/contractAccess'
...
class ManageVitrina extends React.Component {
    state = {
        spacing: "16",
        open: false,
        name: "",
    ...
    async createStorefrontClicked() {
        ...
        if (tx.events.LogNewStorefrontCreated) {
            ...
            storefrontsCopy.push({
                owner: storeFrontOwner,
                name: name,
                balance: balance,
                productCount: productCount,
                index: storefrontsCopy.length
            });
                ...
                <div>
                    <Paper className={classes.requestPaper} elevation={1}>
                        ...
                        <Button variant="contained" color="primary" className={classes.button}
                                onClick={() => this.createShopOwnerRequest()}>
                            Create request
                        </Button>
                    </Paper>
                </div>
        ...
        return (
            <div>
                <Paper className={classes.paper}>
                    ...
                    <Button variant="contained" disabled={maxStorefrontCountReached} className={classes.button}
                            onClick={() => this.createStorefrontClicked()}>
                        Create
                    </Button>
                </Paper>
                <br/>
                                            ...
                                            <Grid item xs={12} sm container>
                                                <Grid item xs container direction="column" spacing={16}>
                                                    <Grid item xs>
                                                        ...
                                                        <Typography
                                                            color="textSecondary">Owner: {storefront.owner}</Typography>
                                                    </Grid>
                                                    <Grid item>
                                                        <Button variant="contained" color="primary"
                                                                className={classes.button}
                                                                onClick={() => this.manageProductsClicked(storefront.name, storefront.owner, storefront.index)}>
                                                            Manage products
                                                        </Button>
                                                        <Button variant="contained" color="secondary"
                                                                className={classes.button}
                                                                onClick={() => this.removeStorefrontClicked(storefront.owner, storefront.index)}>
                                                            Remove store
                                                        </Button>
                                                    </Grid>
                                                </Grid>
                                                <Grid item>
                                                    <Typography variant="subtitle1">Product
                                                        count: {storefront.productCount}</Typography>
                                                    <Typography
                                                        variant="subtitle1">Balance: {web3.utils.fromWei(storefront.balance.toString(), "ether")} Ξ</Typography>
                                                </Grid>
                                            </Grid>
                                        </Grid>
                                    </Paper>
...
ManageVitrina.propTypes = {
    classes: PropTypes.object.isRequired
};

ManageVitrina.contextTypes = {
...
const mapDispatchToProps = (dispatch) => {
    return {
        storefrontSelected(storefront) {
            dispatch(Actions.storefrontSelected(storefront))
        },
        pageSelected(page) {
            dispatch(Actions.pageSelected(page))
        },
        userStatusUpdated(userStatus) {
            dispatch(Actions.userStatusUpdated(userStatus))
        }
    }
};
...
export default withRouter(connect(
    mapStateToProps,
    mapDispatchToProps
)(withStyles(styles)(ManageVitrina)))
